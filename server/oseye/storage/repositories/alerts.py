...
class SQLAlertRepository:
    ...
    async def list_by_entity(self, entity_id: str, limit: int = 500) -> list[Alert]:
        """Return alerts for a specific entity_id, newest first.

        H-12: capped at *limit* rows (default 500) to prevent unbounded result
        sets on high-activity entities.
        """
        async with self._session_factory() as session:
            rows = (
                await session.execute(
